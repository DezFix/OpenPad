# OpenPad Virtual Mic — TODO (как реально попробовать драйвер)

> Статус CI: ✅ `.github/workflows/driver-build.yml` собирает чистый
> SysVAD Microsoft в облаке (windows-2022 + msbuild, без локальных
> VS/WDK). Артефакт `sysvad-x64-release` (~18 МБ): `TabletAudioSample.sys`
> компилируется и тест-подписывается. Нюанс: `RunInfVerif=false`, т.к.
> на runner'е нет `x86\InfVerif.dll`.
> Python-сторона уже готова (авто-детект имён `*openpad*`).
